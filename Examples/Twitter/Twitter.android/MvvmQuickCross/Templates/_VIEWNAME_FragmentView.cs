...
namespace Twitter
{
    public class _VIEWNAME_View : FragmentViewBase<_VIEWNAME_ViewModel>
    {
        ...
        {
            TwitterApplication.Instance.ContinueTo_VIEWNAME_(skipNavigation: true); // This line is only needed if this view can be navigated to by some other means than through ContinueTo_VIEWNAME_() 
            var view = inflater.Inflate(Resource.Layout._VIEWNAME_View, container, false);
            Initialize(view, TwitterApplication.Instance._VIEWNAME_ViewModel, inflater);
            return view;
        }
    }
}
